...
class RMSNorm(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        if modulation.ndim == x.ndim - 1:
            # Native pi0.5 supplies one condition per batch item and broadcasts
            # it across the action-token sequence.
            modulation = modulation[:, None, :]
        elif modulation.ndim != x.ndim:
            raise ValueError(
                f"AdaRMS condition must be batch-wide or token-wise, got x.ndim={x.ndim}, "
